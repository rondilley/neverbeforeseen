...
int loadFile(const char *fName) {
  ...
  if (fscanf(inFile, "VER=%d\n", &fileVersion) EQ 1) {
    ...
    if (fileVersion EQ 1) {
      ...
      fprintf(stderr,
              "ERR - File version format [%s] is not compatible with this "
              "version of difftree\n",
              inBuf);
      deInitParser();
      fclose(inFile);
    ...
    for (i = 0; i < ret; i = i + 2) {
      ...
      if (strlen(inBuf) <= 0) {
        ...
        if (strncmp(inBuf, "QUICK", sizeof(inBuf)) EQ 0) {
          if (!config->quick) {
            fprintf(stderr,
                    "ERR - The file's mode is not compatible with the current "
                    "mode [%s->%s], switching to -q|--quick\n",
          ...
          if (config->hash) {
            fprintf(stderr,
                    "ERR - The file's mode is not compatible with the current "
                    "mode [%s->HASH], disabling -m|--md5 or -s|--sha256\n",
                    inBuf);
            config->hash = FALSE;
            config->md5_hash = FALSE;
            config->sha256_hash = FALSE;
        ...
        if (strlen(inBuf) <= 0) {
          ...
          if (config->debug >= 5)
            printf("DEBIG - PERM=%x%x%x%x\n", tPerm >> 9 & S_IRWXO,
                   tPerm >> 6 & S_IRWXO, tPerm >> 3 & S_IRWXO, tPerm & S_IRWXO);
#endif
          ...
          if (config->hash) {
            if (!config->md5_hash) {
              fprintf(stderr, "WARN - Loading metadata file with MD5 hashes, "
                              "switching to MD5 (-m)\n");
          ...
          for (dPos = 0, lPos = 0; dPos < config->digest_size;
               dPos++, lPos += 2) {
            hByte[0] = inBuf[lPos];
          ...
          if (config->hash) {
            if (!config->sha256_hash) {
              fprintf(stderr, "WARN - Loading v1 metadata file with SHA256 "
                              "hashes, switching to SHA256 (-s)\n");
...
#endif
      count++;
      if ((config->exclusions = (char **)XREALLOC(
               config->exclusions, sizeof(char *) * count)) EQ NULL) {
        /* XXX problem */
      }
